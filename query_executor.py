...
#    e) Local Parse ("localParse"): For parsing and extracting data from existing local CSV files.
#    f) Local SQL ("localSQL"): For extracting table and field names from SQL queries in local CSV files.
...
SF_API_VERSION = "v64.0" # Pode ser mantida aqui ou movida para o .env
...
BULK_JOB_POLL_INTERVAL_SECONDS = 5
MAX_WORKERS = 5

proxies = {'http': PROXY_URL, 'https': PROXY_URL} if USE_PROXY else None

# --- MODIFICADO: Suprime os avisos de requisição HTTPS não verificada ---
if not VERIFY_SSL:
...
class SalesforceAuthenticator:
    ...
    def refresh_credentials(self) -> Optional[Tuple[str, str]]:
        tqdm.write("🔄 Refreshing expired access token...") # Usa tqdm.write para não quebrar a barra de progresso
...
def stream_all_ssot_pages(auth: SalesforceAuthenticator, start_url: str, entry_point: str, object_name_for_key: str, api_fields: List[str], filter_conditions: Optional[List[Dict]]) -> Generator[List[Dict], None, None]:
    ...
    # --- MODIFICADO: Lógica de log e erro movida para fora do laço para um melhor tratamento. ---
...
def execute_ssot_query(auth: SalesforceAuthenticator, api_version: str, pseudo_soql_query: str, output_file: str) -> Tuple[int, int]:
    ...
    try:
        ...
        if source_iteration_file and param_names_for_request and source_param_columns:
            # --- Iterative Mode ---
            full_source_filename = f"{source_iteration_file}{OUTPUT_FILE_SUFFIX}.csv"
...
def execute_local_parse_query(query: str, output_file: str) -> Tuple[int, int]:
    ...
    try:
        ...
        source_filename = f"{source_file_base}{OUTPUT_FILE_SUFFIX}.csv"
    ...
    try:
        ...
        for row in queries:
            ...
            try:
                ...
                file_name = f"{object_api_name_for_file}{OUTPUT_FILE_SUFFIX}.csv"
